...
class TemplateLibrariesDao extends DatabaseAccessor<AppDatabase>
    with _$TemplateLibrariesDaoMixin {
  TemplateLibrariesDao(super.db);

  Future<int> createTemplateLibrary(String name) {
    return into(templateLibraries)
        .insert(TemplateLibrariesCompanion(name: Value(name)));
  }

  Future<void> updateTemplateLibrary(int id, String name) {
    return update(templateLibraries)
        .replace(TemplateLibrary(id: id, name: name));
  }

  Stream<TemplateLibrary> watchTemplateLibraryWithId(int id) {
    return (select(templateLibraries)..where((li) => li.id.equals(id)))
        .watchSingle();
  }
